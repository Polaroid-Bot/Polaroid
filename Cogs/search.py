...
class search(commands.Cog):
    ...
    async def search(self, ctx, *, query):
        query = query.replace(' ', '+')
        async with self.ses.get(f"https://pixabay.com/api/?key={self.pixabay}&q={query}&image_type=photo&safesearch=true") as r:
    ...
    async def art(self, ctx, *, query):
        query = query.replace(' ', '+')
        async with self.ses.get(f"https://pixabay.com/api/?key={self.pixabay}&q={query}&image_type=illustration&safesearch=true") as r:
            if r.status in range(200, 299):
                try:
                    data = await r.json()
                    image_url = data["hits"][0]["webformatURL"]
